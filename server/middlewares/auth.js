  ...
  try {
    ...
    try {
      ...
      if (!req.user.emailVerified) {
        return res.status(403).json({
          success: false,
          message: 'Please verify your email address to access this feature. Check your inbox for the verification code.',
...
// Protect routes but allow unverified users (for email verification endpoints)
exports.protectAllowUnverified = async (req, res, next) => {
  try {
    let token;
    ...
    try {
      ...
      // Skip email verification check - allow unverified users
      next();
    } catch (error) {
      return res.status(401).json({
        success: false,
